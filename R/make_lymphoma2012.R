...
eset.focal<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_focal.RDS", sep = ""))
eset.focal.binary<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_focal_binary.RDS", sep = ""))
eset.arm<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_arm.RDS", sep = ""))
eset.arm.binary<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_arm_binary.RDS", sep = ""))
eset.or<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_or.RDS", sep = ""))
eset.or.binary<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_all_or_binary.RDS", sep = ""))

eset.gep<-readRDS(paste(f.dir.in, "/", "cancercell2012_gep.RDS", sep = ""))
eset.gep.log<-readRDS(paste(f.dir.in, "/", "cancercell2012_gep_log.RDS", sep = ""))
eset.gep.log.corrected<-readRDS(paste(f.dir.in, "/", "cancercell2012_gep_log_correct.RDS", sep = ""))
eset.cisgenes<-readRDS(paste(f.dir.in, "/", "cancercell2012_gistic_cisgenes.RDS", sep = ""))
...
samples.map<-read.table("/Users/amyli/Desktop/git_projects/datasets/lymphoma2010/snp2express.mapping.lymphoma2010_wold.txt",
	sep = "\t", header = TRUE)
...
samples.map$wes_id<-match(samples.map$snp.array.ID, samples.cn)
samples.map$gep_id<-match(samples.map$mrna.array.ID, samples.gep)

samples.map.sub<-subset(samples.map, !is.na(wes_id) & !is.na(gep_id))

#focal annotation, arm sample status
...
saveRDS(lymphoma2012, file = paste(f.dir.in,"/", "cancercell2012_all.RDS", sep = ""))
